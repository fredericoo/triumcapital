...
const Picture: React.FC<ImageProps & PictureProps> = ({ bg = 'gray.100', ...props }) => {
  ...
  const fixImageSrc = (src: string): string => src.replace('auto=compress,format', '');
  ...
  return (
    <Box transition="background-color 1s ease-out" bg={hasLoaded ? 'transparent' : bg} className={props.className}>
