...
type BaseSlice = { items: Record<string, unknown>[]; primary: Record<string, unknown>; slice_type: string };
...
const Slices: React.FC<SlicesProps> = ({ body }) => {
  const renderSlice: Record<string, (slice: Slice) => JSX.Element> = {
    ...
    embed_de_video: ({ primary }: EmbedSlice) => <EmbedSlice content={primary.url} />,
  ...
  return (
    <Grid
      ...
      rowGap={{ base: 8, md: 0 }}
    >
      {body.map((slice: Slice) => renderSlice[slice.slice_type] && renderSlice[slice.slice_type](slice))}
    </Grid>
  );
};
